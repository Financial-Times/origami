...
async function runCmd(file, args = [], options = {}) {
...
const rawArg = process.env.RELEASE_OUTPUT || "{}";
let outputs = {};
try {
	...
	console.error("RELEASE_OUTPUT:", rawArg);
	process.exitCode = 2;
	process.exit();
}
...
async function run() {
	...
	for (const workspace of pathsReleased) {
		...
		const releaseCreated =
			String(outputs[`${workspace}--release_created`] ?? "").toLowerCase() ===
			"true";
		if (!releaseCreated) {
