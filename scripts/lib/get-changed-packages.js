...
export async function getChangedPackages(baseRef, headRef) {
	...
	let commitHashesAndMessages = await commitHashesAndMessagesBetween(
		baseRef,
		headRef
	);
...
async function getPublishablePackages() {
	...
	try {
		releasePleaseFile = await fs.readFile(
 ...
 */
async function commitHashesAndMessagesBetween(
	baseRef = "main",
	headRef = "HEAD"
) {
	$.verbose = false;
	let commitHashesAndMessages = (
		await $`git log --pretty="format:%H %s" ${headRef} --not ${baseRef} --`
	).stdout.split("\n");
	$.verbose = true;
...
async function findModifiedFilesInCommit(commitSha) {
	$.verbose = false;
	let filesChanged = (
		await $`git show --pretty="format:" --name-only ${commitSha}`
	).stdout.split("\n");
	$.verbose = true;
	return filesChanged;
}
