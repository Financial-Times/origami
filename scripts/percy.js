...
async function shouldPercyRun() {
	...
	if (isDefaultBranch) {
		core.notice(
			"This is a commit on the default branch, we need to run Percy to update the baseline images."
		);
		return true;
		...
		$.verbose = false;
		let commits =
			await $`git log --pretty=format:%s origin/${baseRef}...origin/${headRef}`;
		$.verbose = true;
		...
		for (const message of messages) {
			if (
				...
				$.verbose = false;
				let { stdout: changedFiles } =
					await $`git diff --name-only origin/${baseRef}...origin/${headRef}`;
				$.verbose = true;
				...
				if (changedPackages.has(pkg.name)) {
					core.notice(
						"A commit in the pull-request is a `fix` or `feat` commit - which means some user facing changes have been made. Those user facing changes are in this package. This means we need to run Percy on the pull-request."
					);
					return true;
				}
				const packageDependencies = new Set(
				...
				for (const dependency of packageDependencies) {
					if (changedPackages.has(dependency)) {
						core.notice(
							"A commit in the pull-request is a `fix` or `feat` commit - which means some user facing changes have been made. Those user facing changes are in a dependency of this package. This means we need to run Percy on the pull-request."
						);
						core.notice(
							`The dependencies for this package (${
								pkg.name
							}) are: ${packageDependencies.join("\n")}`
						);
						core.notice(
							`The packages which were affected by the user facing change are: ${Array.from(
								changedPackages.keys()
							).join("\n")}`
						);
						return true;
					}
		...
		core.notice(
			"No commits in the pull-request are a `fix` or `feat` - which means no user facing changes have been made. This means we do not need to run Percy."
		);
		return false;
	} else {
		core.notice(
			"We are not running Percy because this is not a pull-request or a commit on the default branch."
		);
		return false;
	}
}
...
try {
	if (!process.env.PERCY_TOKEN || process.env.PERCY_TOKEN == "") {
		throw new Error(
			"The percy token is not set, please add the token in the GitHub Actions Secrets. You should be able to find the token in the project on the Origami Percy account (https://percy.io/27a07468). If a project does not exist, you should be able to create a new project (https://percy.io/organizations/27a07468/projects/new)."
		);
	}
	...
	if (percyNeedsToRun) {
		...
		if (demosConfig.length === 0) {
			core.notice(
				"We are not running Percy because there are no demos for this component."
			);
			process.exit(0);
		}
