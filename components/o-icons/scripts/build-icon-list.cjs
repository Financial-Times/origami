...
function runScript() {
	let {images} = loadJson(
		`${__dirname}/../../../node_modules/@financial-times/fticons/imageset.json`
	);
	...
	writeTsInterface(images, `${__dirname}/../src/tsx/data.ts`);
}
...
function writeTsInterface(images, filePath) {
	const iconNames = images.map(image => image.name);
