...
const mixHexes = (mixer, base) => {
	...
	return percentages.map(percent => {
		...
		for (let i = 0; i <= 5; i += 2) {
			...
			let combinedPair = decimalToHex(
				Math.round(basePair + (mixPair - basePair) * (percent / 100.0))
			);

			while (combinedPair.length < 2) {
				combinedPair = `0${combinedPair}`;
			} // prepend a '0' if combinedPair results in a single digit
