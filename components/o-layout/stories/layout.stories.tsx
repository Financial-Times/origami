...
const HeaderServicesStory = args => {
	useEffect(() => void headerServicesJS.init(document.body), [])
...
export default {
	...
	args: {
		...
		footer: <DemoFooter />,
		headerControls: HeaderServicesStory,
	},
	argTypes: {
...
export function defaultControlsToProps(args) {
	args.header = <HeaderServicesStory {...args.headerControls} />
	return args
}

export const DefaultLayout: StoryObj<LayoutStoryProps> = {
	render: args => {
		useEffect(() => {
			Table.init()
...
export const DocumentationLayout: StoryObj<DocsLayoutStoryProps> = {
	render: args => {
		useEffect(() => {
			Table.init()
			SyntaxHighlight.init()
