						...
						source: function customSuggestions(query, populateOptions) {
							...
							suggestionTimeoutId = setTimeout(() => {
								...
								for (const suggestion of suggestions) {
									const lowercaseSuggestion = suggestion.toLocaleLowerCase();
									if (
										lowercaseSuggestion.startsWith(query.toLocaleLowerCase())
									) {
										filteredOptions.push(suggestion);
									}
								}
		...
		context(
			...
			() => {
				...
				beforeEach(() => {
					fixtures.htmlInputCode();
					onConfirm = sinon.spy();
					source = sinon.spy(function customSuggestions(
						query,
						populateOptions
					) {
						const suggestions = [
					...
					new Autocomplete(
						document.querySelector('[data-o-component="o-autocomplete"]'),
						{
							source: function customSuggestions(query, populateOptions) {
								...
								const filteredOptions = [];
								for (const suggestion of suggestions) {
									const lowercaseSuggestion =
										suggestion.team.toLocaleLowerCase();
									if (
										lowercaseSuggestion.startsWith(query.toLocaleLowerCase())
									) {
										filteredOptions.push(suggestion);
				...
				it("throws an error indicating that a mapOptionToSuggestedValue needs to be defined in order to convert the options into strings", async () => {
					...
					window.onunhandledrejection = event => {
						assert.equal(
							...
							'The option trying to be displayed as a suggestion is not a string, it is "object". o-autocomplete can only display strings as suggestions. Define a `mapOptionToSuggestedValue` function to convert the option into a string to be used as the suggestion.'
						);
