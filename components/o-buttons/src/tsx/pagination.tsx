...
function splitPages(pages) {
  ...
  if(currentPage.number < 3 || currentPage.number >= secondPageNumberFromEnd) {
    return [pagesBetweenNumbers(0, 3), pagesBetweenNumbers(thirdPageNumberFromEnd, lastPageNumber)];
  }

  if(currentPage.number === 3) {
    return [pagesBetweenNumbers(0, 4), pagesBetweenNumbers(secondPageNumberFromEnd, lastPageNumber)];
  }

  if(currentPage.number === thirdPageNumberFromEnd) {
    return [pagesBetweenNumbers(0, 2), pagesBetweenNumbers(fourthPageNumberFromEnd, lastPageNumber)];
  }

  const threeOrMoreIn = currentPage.number <= thirdPageNumberFromEnd && currentPage.number >= 3;
  if(threeOrMoreIn) {
    return [pagesMatchingNumber(1), pagesBetweenNumbers(previousPage, nextPage), pagesMatchingNumber(lastPageNumber)];
  }
}
...
export function ButtonPagination({
  ...
  const pageElementsInGroups = pagesToDisplayInGroups.map(pageGroup => pageGroup.map(page => {
    ...
    return (
        <div className="o-buttons-pagination">
          <PreviousTag attributes={firstPageIsSelected ? {disabled: true} : {onClick: previousPager.onClick}} href={previousPager.href} label={previousPager.label} icon={'arrow-left'} iconOnly={true} theme={theme} size={size} type={type}></PreviousTag>
          ...
          <NextTag attributes={lastPageIsSelected ? {disabled: true} : {onClick: nextPager.onClick}} href={nextPager.href} label={nextPager.label} icon={'arrow-right'} iconOnly={true} theme={theme} size={size} type={type}></NextTag>
        </div>
        );
    }
