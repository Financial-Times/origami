class MultiSelect {
	...
	constructor (multiSelectEl, options) {
		...
		}, options || MultiSelect.getDataAttributes(multiSelectEl));
	}
	/**
	...
	static getDataAttributes (multiSelectEl) {
		...
		return Object.keys(multiSelectEl.dataset).reduce((options, key) => {
			...
			const shortKey = key.replace(/^oMultiSelect(\w)(\w+)$/, (m, m1, m2) => m1.toLowerCase() + m2);
	...
	static init (rootElement, options) {
		...
		return Array.from(rootElement.querySelectorAll('[data-o-component="o-multi-select"]'), rootEl => new MultiSelect(rootEl, options));
	}
}
