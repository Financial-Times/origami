...
function assertBackButton({disabled, visuallyHidden, sticky, docked}) {
	...
	proclaim.equal(
		backElement.classList.contains('o-table-control--hide'),
		visuallyHidden,
		`Back button is ${visuallyHidden ? 'visible' : 'not visible'}.`
	);
	proclaim.equal(
		backButton.hasAttribute('disabled'),
		disabled,
		`Back button is ${disabled ? 'not disabled' : 'disabled'}.`
	);
	if (sticky !== undefined) {
		proclaim.equal(
			backElement.classList.contains('o-table-control--sticky'),
			sticky,
			`Back button is ${sticky ? 'not sticky' : 'sticky'}.`
		);
	}
	if (docked !== undefined) {
		proclaim.equal(
			backElement.classList.contains('o-table-control--dock'),
			sticky,
			`Back button is ${docked ? 'not docked' : 'docked'}.`
		);
	}
}
...
function assertForwardButton({disabled, visuallyHidden, sticky, docked}) {
	...
	proclaim.equal(
		forwardElement.classList.contains('o-table-control--hide'),
		visuallyHidden,
		`Forward button is ${visuallyHidden ? 'visible' : 'not visible'}.`
	);
	proclaim.equal(
		forwardButton.hasAttribute('disabled'),
		disabled,
		`Forward button is ${disabled ? 'not disabled' : 'disabled'}.`
	);
	if (sticky !== undefined) {
		proclaim.equal(
			forwardElement.classList.contains('o-table-control--sticky'),
			sticky,
			`Forward button is ${sticky ? 'not sticky' : 'sticky'}.`
		);
	}
	if (docked !== undefined) {
		proclaim.equal(
			forwardElement.classList.contains('o-table-control--dock'),
			sticky,
			`Forward button is ${docked ? 'not docked' : 'docked'}.`
		);
	}
}
...
function assertExpanded(table, {expanded, minimumRowCount}) {
	...
	const expectedHidden = expanded
		? 0
		: table.tableRows.length - minimumRowCount;
	proclaim.include(
		table.container.innerHTML.toLowerCase(),
		expectedButtonContent,
		`Expected to see "${expectedButtonContent}" within an expanded table.`
	);
	proclaim.isTrue(
		table.rootEl.getAttribute('aria-expanded') === expectedAriaExpanded,
		`Expected to see \'aria-expanded="${expectedAriaExpanded}"\' on an expanded table.`
	);
	proclaim.equal(
		table.tbody.querySelectorAll('tr[aria-hidden="true"]').length,
		expectedHidden,
		`Expected ${expectedHidden} table rows to be hidden in an expanded table,`
	);
	if (expanded) {
		proclaim.isTrue(
			table.wrapper.style.height !== '',
			`Expect an expanded table to have a set height on its wrapper to hide rows visually.`
		);
	}
}
...
describe('OverflowTable', () => {
			...
			window.onerror = function (message, file, line, col, error) {
				try {
					proclaim.include(
						error.message,
						'container',
						'Expected an error when a table is configured to be expandable but has no container element.'
					);
					window.onerror = null;
					done();
					return true;
		...
		it('is not used if the number of rows to contract to is larger than the table', done => {
			...
			setTimeout(() => {
				try {
					proclaim.isFalse(table.canExpand());
					proclaim.isFalse(
						table.rootEl.hasAttribute('aria-expanded'),
						`Did not expect "aria-expanded" on a table which can not expand.`
					);
					done();
				} catch (error) {
					done(error);
	...
	describe('scroll controls', () => {
		...
		it('are not added if there are no wrapper and container elements', done => {
			...
			setTimeout(() => {
				try {
					proclaim.isNull(
						oTableEl.querySelector('.o-table-control--forward'),
						'Did not expect to find a forward button.'
					);
					proclaim.isNull(
						oTableEl.querySelector('.o-table-control--back'),
						'Did not expect to find a back button.'
					);
					done();
				} catch (error) {
					done(error);
		...
		it('forward / backward buttons are added given wrapper and container elements', done => {
			...
			setTimeout(() => {
				try {
					proclaim.isNotNull(
						document.querySelector('.o-table-control--forward'),
						'Did not find forward button.'
					);
					proclaim.isNotNull(
						document.querySelector('.o-table-control--back'),
						'Did not find back button.'
					);
					done();
				} catch (error) {
					done(error);
	...
	describe('control dock', () => {
		function assertDock(hasDock) {
			const controlOverlay = document.querySelector(
				'.o-table-overflow-control-overlay'
			);
			proclaim.equal(
				controlOverlay.classList.contains(
					'o-table-overflow-control-overlay--arrow-dock'
				),
				hasDock,
				`Table control overlay does ${
					hasDock ? 'not have a control dock' : 'has a control dock'
				}.`
			);
		}
