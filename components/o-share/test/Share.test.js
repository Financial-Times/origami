...
describe('component', () => {
	...
	it('generates markup when instantiating a component', () => {
		proclaim.instanceOf(
			document.querySelector('.o-share__icon--twitter'),
			HTMLElement
		);
		proclaim.instanceOf(
			document.querySelector('.o-share__icon--facebook'),
			HTMLElement
		);
		proclaim.instanceOf(
			document.querySelector('.o-share__icon--linkedin'),
			HTMLElement
		);
		proclaim.instanceOf(
			document.querySelector('.o-share__icon--whatsapp'),
			HTMLElement
		);
		proclaim.instanceOf(
			document.querySelector('.o-share__icon--pinterest'),
			HTMLElement
		);
	});
	...
	it('generates descriptive link text', () => {
		const twitterText = document.querySelector(
			'.o-share__icon--twitter .o-share__text'
		).innerText;
		const facebookText = document.querySelector(
			'.o-share__icon--facebook .o-share__text'
		).innerText;
		const linkedinText = document.querySelector(
			'.o-share__icon--linkedin .o-share__text'
		).innerText;
		const whatsappText = document.querySelector(
			'.o-share__icon--whatsapp .o-share__text'
		).innerText;
		const pinterestText = document.querySelector(
			'.o-share__icon--pinterest .o-share__text'
		).innerText;

		proclaim.strictEqual(
			twitterText,
			'Share Test Article on Twitter (opens a new window)'
		);
		proclaim.strictEqual(
			facebookText,
			'Share Test Article on Facebook (opens a new window)'
		);
		proclaim.strictEqual(
			linkedinText,
			'Share Test Article on LinkedIn (opens a new window)'
		);
		proclaim.strictEqual(
			whatsappText,
			'Share Test Article on Whatsapp (opens a new window)'
		);
		proclaim.strictEqual(
			pinterestText,
			'Share Test Article on Pinterest (opens a new window)'
		);
	});
	...
	it('responds correctly to twitter clicks in the component', () => {
		...
		proclaim.strictEqual(
			spy.calledWith[0],
			twitterLinkElement.getAttribute('href')
		);
		proclaim.strictEqual(spy.calledWith[1], '');
		proclaim.strictEqual(spy.calledWith[2], 'width=646,height=436');
	});
...
describe('data normalisation', () => {
	it('converts relative urls to absolute urls before sharing them', () => {
		...
		const twitterHref = decodeURIComponent(
			twitterLinkElement.getAttribute('href')
		);

		proclaim.match(
			twitterHref,
			/url=https?:\/\/localhost:[\d]+\/content\/test/
		);
	});
