...
(function () {
	...
	var startBookmarklet = function ($) {
		...
		if ($("#grid-displayer").length) {
			// Close grid displayer when the bookmarklet is clicked for a second time
			removeGridDisplayer();
		} else {
			// Default parameters
			var gdNbcols = 12;
			...
			var origamis = [
				["o4", "Origami Grid v4"],
				["so4", "Origami Grid v4 (snappy)"],
			];
			var frameworks = origamis;

			// HTML
			var gridHtml =
					'<div id="grid-displayer" style="display: none;"><div class="gd-container"><div class="gd-row"></div></div></div>',
			...
			gridToolsHtml +=
				'	  <div><label for="gdt-color">Columns colour</label><input type="text" id="gdt-color" value="' +
				gdColor +
				'" /></div>';
			gridToolsHtml +=
				'		<div><label for="gdt-opacity">Opacity</label><input type="text" id="gdt-opacity" value="' +
				gdOpacity +
				'" /></div>';
			gridToolsHtml +=
				'		<div class="framework-specific twb"><label for="gdt-nbcols">Nb cols</label><input type="text" id="gdt-nbcols" value="' +
				gdNbcols +
				'" /></div>';
			gridToolsHtml +=
				'		<div><label for="gdt-zindex">z-index</label><input type="text" id="gdt-zindex" value="' +
				gdZindex +
				'" /></div>';
			gridToolsHtml += "	</div>";
			gridToolsHtml +=
			...
			$("head").append(
				"<link rel='stylesheet' href='https://unpkg.com/@financial-times/o-grid@^6.0.0/bookmarklet/bookmarklet.css' />"
			);

			// A version of the grid might already be loaded, so we load it only if needed
			if ($(".o-grid-row").length === 0) {
				$("head").append(
					"<link rel='stylesheet' href='https://www.ft.com/__origami/service/build/v3/bundles/css?components=o-grid@^6.0.0&brand=master&system_code=origami' />"
				);
			}
	...
	gridScript.src =
		"https://www.ft.com/__origami/service/build/v3/bundles/js?components=o-grid@^6.0.0&system_code=origami";
