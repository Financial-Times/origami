...
describe('Core.Send', function () {
	...
	describe('fallback transports', function () {
		...
		it('use sendBeacon by default', function (done) {
			...
			setTimeout(() => {
				try {
					proclaim.equal(navigator.sendBeacon.args[0][0], 'https://runtime-api.ft.com/sync?type=video:seek');
		...
		it('fallback to xhr when config.queue is set to true', function (done) {
			...
			setTimeout(() => {
				try {
					...
					proclaim.ok(dummyXHR.open.calledWith("POST", "https://runtime-api.ft.com/sync?type=video:seek", true), 'is POST');
		...
		it('fallback to image when xhr withCredentials and sendBeacon not supported', function (done) {
			...
			setTimeout(() => {
				try {
					...
					proclaim.equal(domain, "https://runtime-api.ft.com/px.gif");
