...
describe("Utils", function () {
	...
	it("should provide is functionality", function () {
		[
			...
			proclaim.ok(
				is(test.value, test.answer),
				test.value + " is a " + test.answer
			);
		});
	});
	...
	it("should provide merge functionality", function () {
		proclaim.deepEqual(merge({ one: "one" }, { two: "two" }), {
			one: "one",
			two: "two",
		});
	});
	...
	it("should provide encode functionality", function () {
		proclaim.equal(
			encode("http://www.ft.com?foo=bar&testing=yay!"),
			"http%3A%2F%2Fwww.ft.com%3Ffoo%3Dbar%26testing%3Dyay!"
		);
	});
	...
	it("should provide filterProperties functionality", function () {
		...
		[
			...
			proclaim.deepEqual(
				filterProperties(test.target, allowedPropertyNames),
				test.result
			);
		});
	});
