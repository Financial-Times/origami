...
describe('Core', function () {
	...
	describe('track', function () {
		...
		it('should send a tracking request', function () {
			...
			// System
			proclaim.deepEqual(Object.keys(sent_data.system), [
				'version',
				'source',
				'is_live',
			]);
			...
			// Context
			proclaim.deepEqual(Object.keys(sent_data.context), [
				'id',
				'root_id',
				'url',
			]);
			proclaim.ok(
				guid_re.test(sent_data.context.id),
				'Request ID is invalid. ' + sent_data.context.id
			);
			proclaim.equal(sent_data.context.root_id, root_id);
			proclaim.equal(sent_data.context.url, 'http://www.ft.com/home/uk');

			// User
			proclaim.deepEqual(Object.keys(sent_data.user), [
				'ft_session',
				'ft_session_s',
				'user_id',
			]);
			...
			// Device
			proclaim.deepEqual(Object.keys(sent_data.device), [
				'spoor_session',
				'spoor_session_is_new',
				'spoor_id',
			]);
		...
		it('should defer a tracking request', function () {
			...
			proclaim.ok(
				callback.calledOnce,
				'Callback should only be called once as next send could be on a different page.'
			);
		});
	});
});
