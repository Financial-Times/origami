...
(async () => {
	...
	console.log(
		"- Getting all cards from the project board's complete column...\n"
	);
	const cards = [];
	...
	const processedContent = cardsContent
		.filter(
			c =>
				!ignoredContributorTypes.includes(c.user.type) &&
				!ignoredUserLogins.includes(c.user.login)
		)
		.map(c => {
			return {
				title: c.title,
	...
	for (let [repoUrl, content] of Object.entries(singleProjectContent)) {
		...
		content.forEach(c => {
			console.log(
				`   - ${c.title} (opened by: ${c.opened_by}, closed by: ${c.closed_by}, updated at ${c.updated_at})\n`
			);
		});
	}
	...
	console.log("\n### Duplicates\n");
	console.log(
		"These titles were matched in one or more PRs/issues. Either the issue has the same name as a PR or the work was done over multiple projects.\n"
	);
