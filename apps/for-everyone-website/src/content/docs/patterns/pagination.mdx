...
- A similar legacy pagination pattern exists within our [legacy o-buttons component](https://main--655f72ec522e424302dc6201.chromatic.com/?path=/story/o2-core_components-o-buttons--pagination). However, this legacy pattern does not support a mobile experience. It also is not represented in our legacy Figma UI Library.
...
<Example>
    <ButtonPagination previousPager={{label: 'previous', href: '#previous'}} nextPager={{label: 'next', href: '#next'}} pages={[1,2,3,4,5,6,7].map(p => ({href: `#${p}`, current: p === 2, number: p}))} />
</Example>
...
<Example title="Avoid infinite scroll when" do={false}>
    - You're working on the web, where it is not an established pattern for us, unless user research shows they improve your interface.
    - It interferes with access to content below, such as our footer.
    - Where returning to or sharing a given pages is helpful.
</Example>
