...
export async function constructPolyfillUrl() {
	...
	const { stdout: componentsDependencies } = await execa(
		`npx`,
		["npm", "ls", `-w components/${componentName}`],
		{
			cwd: projectRoot,
		}
	);
	// Filter the list of all dependencies to find only those which are other origami components
	const origamiDependencies = componentsDependencies
		.split("\n")
		.filter(line => line.includes("-> ./components/"))
		.map(line => {
			return (
				line.match(/\-> (?<location>.+)/).groups.location + "/origami.json"
			);
		});
	...
	await Promise.all(
		origamiDependencies.map(async dependency => {
			const origami = JSON.parse(
				await readFile(path.resolve(projectRoot, dependency), "utf-8")
			);
	...
	if (features.length > 0) {
		return `https://polyfill.io/v3/polyfill.min.js?features=${features.join(
			","
		)}&flags=gated&unknown=polyfill`;
	} else {
