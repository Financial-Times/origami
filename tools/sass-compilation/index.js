...
for (const brand of brands) {
	console.log(
		`Testing compilation for the ${brand} brand without calling any of the Sass API.`
	);
	await compilationTest(sassMainFile, {silent: true, brand: false});

	console.log(
		`Testing compilation for the ${brand} brand with a call to the primary mixin.`
	);
...
async function compilationTest(
	...
	let sassData = '$system-code: "origami";';
	if (brand) {
	...
	try {
		$.verbose = false;
		const result =
			await $`${sassBinary} ${sassTestPath} --style=compressed --no-source-map ${sassIncludePaths}`;
		const css = result.stdout;

		if (silent && css) {
			throw new Error(
				"CSS was output by default, without making a call to the component's Sass API."
			);
		}
	} catch (error) {
...
function getSassIncludePaths(
	...
	const npmPaths = [join('..', '..', 'node_modules'), 'node_modules'];
