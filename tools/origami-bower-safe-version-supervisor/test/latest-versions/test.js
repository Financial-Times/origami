...
tap.test('project which uses latest origami versions which are only on npm and not on bower', async t => {
	...
	t.equal(exitCode, 1)
	t.equal(stdout, '')
	t.equal(stderr, dedent`This project ("test") is installing versions of Origami dependencies which are not available on Bower.
