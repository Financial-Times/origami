	...
	if (dependencies) {
		for (let [name, versionRange] of Object.entries(dependencies)) {
			...
			if (name in origamiComponents) {
				...
				const lastVersionAvailableOnBower = origamiComponents[name].replace(
					"-bower",
					""
				)
				const isUnbounded =
					versionRange === "*" || !isBounded.range(versionRange)
				if (isUnbounded) {
					dependenciesToDowngrade.push([
						name,
						lastVersionAvailableOnBower,
						versionRange,
					])
				} else {
...
async function main() {
	...
	if (
		dependenciesToDowngrade.length ||
		devDependenciesToDowngrade.length ||
		peerDependenciesToDowngrade.length
	) {
		...
		for (const [name, lastVersionAvailableOnBower, installedVersion] of [
			...dependenciesToDowngrade,
			...devDependenciesToDowngrade,
			...peerDependenciesToDowngrade,
		]) {
		...
		for (const [name, lastVersionAvailableOnBower] of dependenciesToDowngrade) {
			console.error(
				`If this project is a library, use \`npm install --save-exact --save-peer "${name}@${lastVersionAvailableOnBower}"\` otherwise, use \`npm install --save-exact --save "${name}@${lastVersionAvailableOnBower}"\``
			)
		}
		for (const [
			name,
			lastVersionAvailableOnBower,
		] of devDependenciesToDowngrade) {
			console.error(
				`npm install --save-exact --save-dev "${name}@${lastVersionAvailableOnBower}"`
			)
		}
		for (const [
			name,
			lastVersionAvailableOnBower,
		] of peerDependenciesToDowngrade) {
			console.error(
				`npm install --save-exact --save-peer "${name}@${lastVersionAvailableOnBower}"`
			)
		}
