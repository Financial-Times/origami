/* eslint-env mocha */
import {assert, aTimeout as sleep} from '@open-wc/testing';
import sinon from 'sinon/pkg/sinon-esm.js/pkg/sinon-esm.js';
...
describe("Autocomplete", function () {
	...
	it("should autoinitialize", async () => {
	...
	describe('enhanced select element', () => {
		...
		context('input matches a single suggestion', () => {
			...
			it("shows a clear button when text has been input by the user", async () => {
				const autocomplete = new Autocomplete(document.querySelector('[data-o-component="o-autocomplete"]'));
				assert.instanceOf(autocomplete, Autocomplete);
				const input = screen.getByRole('combobox', {
			...
			context('pressing Escape key after typing into the input', () => {
				it("hides the suggestion box", async () => {
					...
					await sleep(100);
					assert.isTrue(list.classList.contains('o-autocomplete__menu--hidden'));
				});
			});
