...
function getBrands(origamiConfig) {
	const hasBrandsDefined =
		origamiConfig &&
		origamiConfig.brands &&
		Array.isArray(origamiConfig.brands) &&
		origamiConfig.brands.length > 0;
	if (hasBrandsDefined) {
...
function getComponentDefaultDemoConfig(origamiConfig) {
	if (
		origamiConfig &&
		origamiConfig.demosDefaults &&
		typeof origamiConfig.demosDefaults === "object"
	) {
...
async function buildDemoHtml(buildConfig) {
	...
	const [moduleName, oDemoTpl] = await Promise.all([
	...
	data.oDemoComponentScriptPath = buildConfig.demo.jsDestination
		? path.basename(buildConfig.demo.jsDestination)
		: "";
	...
	const template = await readFile(
		path.join(__dirname, "/page.mustache"),
		"utf-8"
	);
...
if (!hasDemos(origamiConfig)) {
	console.error(
		`No demos exist in the origami.json file. Reference https://origami.ft.com/documentation/manifests/origami-json/ to configure demos in the component's origami.json manifest file.`
	);
	process.exit(0);
}
...
function demoSupportsBrand(demoConfig, brand) {
	const isNotBrandSpecificDemo =
		!demoConfig.brands || demoConfig.brands.length === 0;
...
for (const brand of brands) {
	...
	for (const demoBuild of demoBuildConfig) {
		...
		if (demoBuild.sass) {
			demoBuild.sassDestination =
				buildConfig.brand +
				"-" +
				path.basename(demoBuild.sass).replace(".scss", ".css");
			if (newSassBuild) {
		...
		if (demoBuild.js) {
			demoBuild.jsDestination =
				buildConfig.brand + "-" + path.basename(demoBuild.js);
			if (newJsBuild) {
