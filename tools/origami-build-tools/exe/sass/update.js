...
async function main() {
	console.debug(
		'Making request to https://api.github.com/repos/sass/dart-sass/releases/latest'
	);
	const latest = await axios.get(
		'https://api.github.com/repos/sass/dart-sass/releases/latest'
	);
	console.debug(
		'Completed request to https://api.github.com/repos/sass/dart-sass/releases/latest'
	);
	...
	for (const platform of platforms) {
		...
		const destination = path.resolve(__dirname, `${platform}`);
