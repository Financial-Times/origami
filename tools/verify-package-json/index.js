...
async function validJavaScriptEntryPoint(manifest, workingDirectory) {
	const mainJavaScriptExpectedPath = path.resolve(
		path.join(workingDirectory, "/main.js")
	);
	const mainJavaScriptFileExists = await files.fileExists(
		mainJavaScriptExpectedPath
	);
	if (mainJavaScriptFileExists) {
		if (typeof manifest.browser === "string") {
			const browserPathIsCorrect =
				path.resolve(manifest.browser) === mainJavaScriptExpectedPath;
...
async function packageJson() {
	...
	if (exists) {
		...
		const invalidExplanation = await validJavaScriptEntryPoint(
			packageJson,
			cwd
		);
		...
		result.push(
			`No package.json file found. To make this an origami component, create a package.json file following the format defined at: https://origami.ft.com/documentation/components/code/#package-management`
		);
	}
	return result;
}
...
if (errors.length > 0) {
	...
	if (isCI) {
		const newLine = "%0A";
		console.log(
			`::error file=${cwd}/package.json,line=1,col=1::${message.replace(
				/\n/g,
				newLine
			)}`
		);
