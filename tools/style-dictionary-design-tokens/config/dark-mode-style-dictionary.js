...
const tokenPath = path.dirname(require.resolve('tokens/package.json'));
...
module.exports = (brand) => {
    return styleDictionary.extend({
        include: [
            path.join(tokenPath, `/${brand}/**/!(*.${mode}).json`)
        ],
        source: [
            path.join(tokenPath, `/${brand}/**/*.${mode}.json`)
        ],
        platforms: {
            ...
            css: {
                ...
                files: [{
                    ...
                    filter: (token) =>
                        token.attributes.category !== 'image' &&
                        token.filePath.indexOf(`.${mode}`) > -1,
                    options: {
