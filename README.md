...
## Arrows
Optionally, an overlay can be displayed to be pointing at a target. The arrow can come out of any of the overlay's four sides and the default position is set as an option. If the overlay doesn't fit next to the trigger in the default position, the module will check if it fits in the opposite position and change it if that is the case. This is not a permanent change, it's only until there is space again in the default position.

Also, keep in mind that overlays with arrows can't be modal.
...
* arrow: It's a JSON object with two options
    * position: From which side of the overlay should the arrow come out of. It has to be'top', 'bottom', 'left' or 'right'. _Default_: 'left'
    * target: What should the arrow point at. It may be different from the trigger, and if the target isn't set, the trigger will be used by default. May it be the trigger or not, a target is needed.
...
* trigger: Valid CSS selector. `o-overlay#init` will pass the trigger to the overlay, so this is not needed in declarative mode
...
For overlays with arrows, having a shaded heading is incompatible with positions 'top' and 'bottom'.
